...
fn test_config() -> SimpleConfig { SimpleConfig { lwe_dim: 256, sigma: 6.4 } }
...
where S: IndexScheme + SchemeMeta + 'static {
    ...
    let ops = [
        ("ins",  10u32, 0x10u8), ("ins",  11, 0x11), ("upd",  3, 0x99),
        ("del",   5u32,    0u8), ("ins",  12, 0x12), ("upd",  7, 0xAA),
        ("del",   2u32,    0u8), ("ins", 100, 0x42),
    ];
    for (op, k, v) in ops {
...
where S: IndexScheme + SchemeMeta + 'static {
    ...
    assert_eq!(client.prepared_per_segment(), vec![n as usize; arity],
    ...
    for (i, (key, val)) in inserted.iter().enumerate() {
        ...
        assert_eq!(client.prepared_per_segment(), vec![remaining as usize; arity],
...
where S: IndexScheme + SchemeMeta + 'static {
    ...
    let ops = [
        ("ins", 50u32, 0xCC), ("upd", 5, 0xDD), ("del", 2u32, 0u8), ("ins", 51, 0x77),
    ];
    for (op, k, v) in ops {
        let key = k.to_le_bytes();
        let delta = match op {
