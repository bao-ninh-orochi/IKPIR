...
//! backends (FrodoPIR and SimplePIR), always in warm-bc mode
//! (precompute_queries refilled per-sample).
//!
//! **Method:** Populate to `TableFull`, build server, set up client, then
//! drive `build_query` via criterion's sampling loop. Before each criterion
//! sample's timing bracket, refill the precomputed-query queue with exactly
//! `iters` slots (Phase B only — no Phase C needed for build_query). This
//! guarantees warm-bc for every timed call without the impractical upfront
//! precompute that would stall at paper-scale configs.
//!
...
//!
//! **Output:** `results/ikpir_client_query.csv`
...
#[command(about = "Measure ikpir-client build_query throughput (warm-bc) via criterion.")]
struct Cli {
    ...
    B::Query: Clone, B::Response: Clone,
{
    ...
    // the probe so its `A` copy is freed before the real bench client below.
    ...
    let query_bytes = {
        ...
        probe_client.build_query(&0u32.to_le_bytes()).wire_byte_size()
    };
    ...
    helpers::print_preamble("client_query", &knobs, &store_state, &geom);
    ...
    let mut idx = 0usize;
    {
        ...
        group.bench_function("client_query", |b| {
            b.iter_custom(|iters| {
                // Refill exactly iters warm-bc slots before the timing bracket.
    ...
    writeln!(
        csv,
        "{},{arity},{num_buckets},{},{},{},{lwe_dim_eff},{},{:.2},{:.2},{:.2},{:.2},{query_bytes},{cps},{row_width},{segment_rows},{db_rows},{db_cols},{:.4}",
...
fn main() {
    ...
    let mut csv = helpers::csv_writer("ikpir_client_query.csv", HEADER);
    ...
    println!("\nResults written to results/ikpir_client_query.csv");
}
