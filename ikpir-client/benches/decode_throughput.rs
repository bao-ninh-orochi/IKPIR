...
//!
//! - **`cold`** — no preprocessing; each `decode` recomputes `c = sᵀ·H`
//!   on the fly. Cost ∝ `arity × lwe_dim × row_width`.
//! - **`warm-b`** — `precompute_queries(batch)` only; queries take the
//!   cheap path but decode still computes `c` inline (the design intent
//!   for dynamic-database benchmarks where Phase C is invalidated by every
//!   mutation).
//! - **`warm-bc`** — both `precompute_queries(batch)` and
//!   `precompute_decodes()`; decode collapses to one vector subtract +
//!   rounding plus the slot scan.
//!
//! **Method:** Build a server-client pair, run the configured
//! preprocessing, pre-build a batch of `(query, response)` pairs
//! (excluding query construction and `server.answer` from the timing),
//! then time `batch` `decode` calls.
//!
//! **Invocation:** `cargo bench --bench decode_throughput` runs a single
//! sensible default config in `cold` mode. Pass `--mode warm-b`,
//! `--mode warm-bc`, `--sweep` for the full matrix, or
...
//! `response_bytes` is deterministic given the params; sampled once per row.
...
enum Mode {
    /// No preprocessing — decode recomputes `c = sᵀ·H` on the fly.
    Cold,
    /// Phase B only (queries cheap, decode still on-the-fly). Matches the
    /// dynamic-database paper scenario where Phase C is skipped.
    WarmB,
    /// Phase B + Phase C — decode is one vector subtract + rounding.
    WarmBc,
}
impl Mode {
...
#[command(about = "Measure ikpir-client decode throughput (queries/sec).")]
struct Cli {
...
fn build<S>(cli: &Cli, num_buckets: u32, bucket_size: u32, value_bits: u32) -> Option<(IkpirServer<S, FrodoPirBackend>, Client, u32)>
where S: MakeStore {
...
fn pre_build_responses<S>(server: &IkpirServer<S, FrodoPirBackend>, client: &mut Client, n_inserted: u32, batch: u32) -> Vec<(Vec<u8>, Response)>
where S: MakeStore {
    (0..batch)
        .map(|i| {
            let key = (i % n_inserted).to_le_bytes().to_vec();
...
fn time_decode_batch(client: &Client, batch: &[(Vec<u8>, Response)]) -> f64 {
    let start = Instant::now();
    for (key, r) in batch {
        let _ = client.decode(key, r).expect("decode");
    }
    let elapsed_s = start.elapsed().as_secs_f64();
    batch.len() as f64 / elapsed_s
}

/// Apply the configured preprocessing mode. Critical to call BEFORE
/// `pre_build_responses`: each `build_query` consumes a prepared slot,
/// so the queue must be filled to size `>= batch` first.
...
where S: MakeStore {
    ...
    let batch = pre_build_responses(&server, &mut client, n_inserted, cli.batch);
    let response_bytes = batch[0].1.wire_byte_size();

    for _ in 0..cli.warmup { let _ = time_decode_batch(&client, &batch); }

    let mut samples = Vec::with_capacity(cli.trials as usize);
    for _ in 0..cli.trials {
        samples.push(time_decode_batch(&client, &batch));
    }
    let s = helpers::compute_stats(&samples);
    writeln!(
        csv,
        "{mode_label},{arity},{num_buckets},{bucket_size},{value_bits},{},{:.2},{:.2},{:.2},{:.2},{response_bytes}",
...
fn main() {
    ...
    let mut csv = helpers::csv_writer(
        ...
        "mode,arity,num_buckets,bucket_size,value_bits,batch,mean_qps,min_qps,max_qps,stddev_qps,response_bytes",
    );

    println!("=== ikpir-client decode throughput (FrodoPirBackend) ===");
    println!(
    ...
    // `--sweep --mode warm-bc` pin a single mode.
