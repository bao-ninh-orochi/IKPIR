//! **Intent:** Measure client-side `decode` throughput across both
...
//! batch, mean_qps, min_qps, max_qps, stddev_qps, response_bytes
...
use ikpir_client::{
    ...
    IndexPirBackend, PirResponseBundle, PrecomputingPirBackend, SimpleConfig, SimplePirBackend,
};
    ...
    B::Query: Clone, B::Response: Clone,
{
    ...
    let client_cell = RefCell::new(IkpirClient::<B>::from_setup(server.setup()));
    {
    ...
    let crit = helpers::run_criterion_throughput_batched(
        ...
        |slot: &mut (Vec<u8>, PirResponseBundle<B>)| {
            let (key, r) = slot;
