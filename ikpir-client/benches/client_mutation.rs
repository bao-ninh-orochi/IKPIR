    ...
    B: IndexPirBackend + IncrementalPirBackend,
{
    ...
    for i in 0..cli.n_mutations {
        ...
        match res {
            Ok(bundle) => { deltas.push(bundle); n_succeeded += 1; }
    ...
    B::Query: Clone, B::Response: Clone,
{
    ...
    let geom = helpers::Geometry {
        hint_per_seg_bytes: 0, setup_bundle_bytes: bundle.wire_byte_size(),
    ...
    for &kind in MutationKind::all() {
        ...
        println!(
            ...
            cli.backend, kind.as_csv(), cli.n_mutations, ops_per_s, total_ms,
        );
    }
}
