...
use super::{
    ...
    sample_discrete_gaussian_into, sample_uniform_zq_into,
};
...
impl IndexPirBackend for SimplePirBackend {
    ...
    fn server_setup(
        ...
        let hint_data = compute_hint(
            &a, db, n_rows, row_width, k, lwe_dim, reshape_row_width,
        );
    ...
    fn client_query(state: &mut SimpleClientState, row: u32) -> SimpleQuery {
        ...
        state.in_flight = Some(InFlightSlot { slot, bucket_within });
        SimpleQuery { b }
    }
    ...
    fn server_answer(
        ...
        for r in 0..n_rows as usize {
            ...
            for j in 0..row_width_us {
                a[off_within + j] = a[off_within + j]
                    .wrapping_add(qi.wrapping_mul(db[row_off + j]));
            }
        }
        SimpleResponse { a }
    }
    ...
    fn client_decode(state: &SimpleClientState, response: &SimpleResponse) -> Vec<u32> {
        ...
        let inflight = state.in_flight.as_ref()
...
fn reshape_dims(n_rows: u32, row_width: u32) -> (u32, u32, u32) {
    ...
    let reshape_row_width = k.checked_mul(row_width)
...
#[inline]
fn translate(orig_row: u32, orig_off: u16, k: u32, row_width: u32) -> (u32, u32) {
    ...
    fn client_precompute_decodes(state: &mut SimpleClientState) {
        ...
        if let Some(inflight) = state.in_flight.as_mut() {
            if inflight.slot.c.is_none() {
                inflight.slot.c = Some(
                    compute_c(&inflight.slot.secret, h, lwe_dim, reshape_row_width),
                );
            }
        }
    }
    ...
    I: IntoIterator<Item = &'a mut PreparedSlot>,
{
    ...
    for slot in slots {
        ...
        for (orig_row, cells) in row_deltas {
            ...
            let a_row = &a[(reshape_row as usize) * lwe_dim_us
                ..(reshape_row as usize + 1) * lwe_dim_us];
            ...
            for (orig_off, delta) in cells {
                ...
                c[reshape_off as usize] = c[reshape_off as usize]
                    .wrapping_add(dot.wrapping_mul(delta_u32));
            }
        }
    }
}
...
mod tests {
    ...
    fn translate_handles_reshape_offsets_beyond_u16() {
        ...
        assert_eq!(ro, last_bucket_within * row_width + (last_off_in_row as u32));
        assert!(ro > u16::MAX as u32,
    ...
    fn roundtrip_all_rows(n_rows: u32, row_width: u32) {
        ...
        for row in 0..n_rows {
            ...
            let expected: Vec<u32> = db[row as usize * row_width as usize
                ...
                .to_vec();
            assert_eq!(decoded, expected, "row {row} mismatch (n={n_rows}, w={row_width})");
        }
    }
    ...
    fn patched_c_matches_recomputed_c() {
        ...
        for (slot, secret) in state.prepared.iter().zip(secrets.iter()) {
            let oracle = compute_c(
                secret, &h_patched, sp.params.lwe_dim as usize, sp.reshape_row_width as usize,
            );
    ...
    fn precompute_decodes_fills_in_flight_slot() {
        ...
        assert!(state.in_flight.as_ref().unwrap().slot.c.is_none(),
            "fresh inline-sampled slot starts with no c");

        SimplePirBackend::client_precompute_decodes(&mut state);
        assert!(state.in_flight.as_ref().unwrap().slot.c.is_some(),
