...
//! ring) and `Z_q = Z_{2^32}` (the SimplePIR ciphertext ring). The
//! one-line description: multiply / divide by `Δ = q / p` with
//! round-to-nearest on the way back.
//!
//! # Design / architecture
//!
//! Identical math to `frodo/arith.rs` — duplicated rather than shared
//! per the project rule "don't refactor beyond what the task requires".
//!
//! - `round_p_to_q(x)` = `x << (32 - plaintext_bits)`.
//! - `round_q_to_p(y)` = `⌊(y + Δ/2) / Δ⌋` with wrapping `+ Δ/2`.
//!
//! Both functions are `#[inline]` and run in `O(1)`; they are called
//! once per cell on the SimplePIR hot path.
//!
//! # Related files
//!
...
#[inline]
pub fn round_p_to_q(x: u32, plaintext_bits: u32) -> u32 {
...
#[inline]
pub fn round_q_to_p(y: u32, plaintext_bits: u32) -> u32 {
    debug_assert!((1..=31).contains(&plaintext_bits));
