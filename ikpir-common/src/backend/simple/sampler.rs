...
pub(crate) fn sample_uniform_zq_into<R: RngCore>(rng: &mut R, dst: &mut [u32]) {
...
/// 2 samples).
pub(crate) fn sample_discrete_gaussian_into<R: RngCore>(rng: &mut R, sigma: f64, dst: &mut [u32]) {
...
mod tests {
    ...
    fn gaussian_saturation_branch_untaken() {
        ...
        for &x in &dst {
            ...
            assert!(s.abs() < 200, "gaussian sample {s} out of plausible σ=6.4 range");
        }
    }
