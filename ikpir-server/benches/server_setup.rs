...
#[derive(clap::Parser)]
#[command(about = "Measure ikpir-server setup wall-clock cost (full IkpirServer::new over all \
...
struct Cli {
    ...
    #[arg(long, default_value_t = 0)]      warmup: u32,
    #[arg(long, default_value_t = 1)]      trials: u32,
    ...
    #[arg(long, default_value_t = false)]  estimate: bool,
}
    ...
    B: IndexPirBackend + IncrementalPirBackend + BackendWireSize,
{
    ...
    let (seed_store, n_inserted) = helpers::populate_until_full::<S>(
        num_buckets, cli.bucket_size, cli.fingerprint_bits, cli.value_bits, pb,
    );
    ...
    let knobs = [
        ...
        helpers::Knob { name: "setup_mode",       value: setup_mode_str.to_string(),       is_default: !cli.estimate },
    ];
    ...
    println!(
        ...
        est_note, bundle_bytes, hint_bytes, sp_bytes,
    );
}
