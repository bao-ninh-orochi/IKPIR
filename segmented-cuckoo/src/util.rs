...
pub fn next_power_of_2(x: u64) -> u64 {
    ...
    assert!(
        x <= 1u64 << 63,
        "next_power_of_2 input exceeds u64::MAX/2 ({x})"
    );
    x.next_power_of_two()
}
...
/// hot path.
pub const fn next_power_of_3(x: u64) -> u64 {
    if x <= 1 {
        return 1;
    }
    let mut p = 1u64;
...
pub const fn next_power_of_4(x: u64) -> u64 {
    if x <= 1 {
        return 1;
    }
    let mut p = 1u64;
...
pub const fn is_power_of_3(n: u32) -> bool {
    if n == 0 {
        return false;
    }
    let mut v = n;
...
pub const fn is_power_of_4(n: u32) -> bool {
