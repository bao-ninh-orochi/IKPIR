...
pub(crate) const MAX_KICKS_DEFAULT: u32 = 500;
...
#[inline]
pub(crate) fn target_load_factor(arity: usize, bucket_size: usize) -> f64 {
        ...
        for _ in 0..self.max_kicks {
            ...
            for p in 0..arity {
                ...
                if let Some(_ins_slot) =
                    self.table.insert(all[p], evicted_fingerprint)
                {
                    self.num_items += 1;
                    placed = true;
                    break;
...
mod tests {
    ...
    fn repeated_inserts_reuse_chain() {
        ...
        assert_eq!(f.chain.capacity(), initial_cap, "filter chain grew past max_kicks");
    }
