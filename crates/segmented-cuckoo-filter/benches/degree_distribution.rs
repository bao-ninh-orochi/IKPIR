...
//!
//! **Output:**
//! - `segmented-cuckoo-filter/results/degree_per_bucket.csv` — columns: scheme, arity, num_buckets, bucket_size, bucket_index, degree
//! - `segmented-cuckoo-filter/results/degree_distribution.csv` — columns: scheme, arity, num_buckets, bucket_size, degree, count

mod helpers;

use segmented_cuckoo_filter::{
    CuckooError, IndexScheme, Segmented3aryCuckooFilter, Segmented3aryScheme,
    Segmented4aryCuckooFilter, Segmented4aryScheme, Segmented2aryCuckooFilter, Segmented2aryScheme,
    Standard3aryCuckooFilter, Standard3aryScheme, Standard4aryCuckooFilter, Standard4aryScheme,
    Standard2aryCuckooFilter, Standard2aryScheme,
};
use std::io::Write;
...
fn write_histogram(
    ...
    for &(deg, count) in hist {
        writeln!(csv, "{},{},{},{},{},{}", scheme, arity, num_buckets, bucket_size, deg, count).unwrap();
    }
}
...
fn write_bucket_degrees(
    ...
    for (idx, &deg) in degree.iter().enumerate() {
        writeln!(csv, "{},{},{},{},{},{}", scheme, arity, num_buckets, bucket_size, idx, deg).unwrap();
    }
}
...
fn main() {
    ...
    let mut csv_hist =
        helpers::csv_writer("degree_distribution.csv", "scheme,arity,num_buckets,bucket_size,degree,count");
        ...
        2 => {
            for &bucket_size in DI_BUCKET_SIZE_VALUES {
                ...
                run_degree_index!(
                    ...
                    Segmented2aryScheme { half: DI_NUM_BUCKETS / 2 },
                    "segmented",
                    2,
                    DI_NUM_BUCKETS,
                    bucket_size
                ...
                run_degree_index!(
                    ...
                    Standard2aryScheme { num_buckets: DI_NUM_BUCKETS },
                    "standard",
                    2,
                    DI_NUM_BUCKETS,
                    bucket_size
    ...
    for &bucket_size in DH_BUCKET_SIZE_VALUES {
        ...
        run_degree_hist!(
            ...
            "Segmented 2-ary",
            Segmented2aryCuckooFilter,
            Segmented2aryScheme { half: DH_NUM_BUCKETS / 2 },
            "segmented",
            2,
            DH_NUM_BUCKETS,
            bucket_size
        ...
        run_degree_hist!(
            ...
            "Standard 2-ary",
            Standard2aryCuckooFilter,
            Standard2aryScheme { num_buckets: DH_NUM_BUCKETS },
            "standard",
            2,
            DH_NUM_BUCKETS,
            bucket_size
