...
fn header(title: &str) {
    ...
    println!(
        ...
        "num_buckets", "bucket_size", "fingerprint_bits", "kicks", "trials", "min_lf", "mean_lf", "max_lf"
    );
    println!("{}", "-".repeat(96));
}
            ...
            for &bucket_size in &[1u32, 2, 3, 4] {
                for &max_kicks in KICKS_VALUES {
                    ...
                    println!(
                        ...
                        num_buckets_label, bucket_size, FINGERPRINT_BITS, max_kicks, TRIALS, stats.min, stats.mean, stats.max
                    );
                }
            }
        }
