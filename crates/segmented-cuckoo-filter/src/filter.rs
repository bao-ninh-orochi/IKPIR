...
//! ## Architecture
//!
//! This module depends on [`crate::bucket::FingerprintTable`] for bit-packed storage and on
...
use crate::bucket::FingerprintTable;
use crate::scheme::{
...
#[inline]
fn target_load_factor(arity: usize, bucket_size: usize) -> f64 {
...
///
/// ```rust
/// use segmented_cuckoo_filter::{CuckooError, CuckooFilter, Segmented2aryScheme};
///
...
///
/// ```rust
/// use segmented_cuckoo_filter::{CuckooFilter, Segmented2aryScheme};
///
...
impl CuckooFilter<Segmented2aryScheme> {
    ...
    /// ```
    pub fn new(num_buckets: u32, bucket_size: u32, fingerprint_bits: u32) -> Result<Self, CuckooError> {
        ...
        Ok(CuckooFilter {
            table,
            scheme: Segmented2aryScheme { half: num_buckets / 2 },
            num_items: 0,
...
impl CuckooFilter<Segmented3aryScheme> {
    ...
    ///
    /// ```rust
    /// use segmented_cuckoo_filter::{CuckooFilter, Segmented3aryScheme};
    ///
    ...
    pub fn new(num_buckets: u32, bucket_size: u32, fingerprint_bits: u32) -> Result<Self, CuckooError> {
        ...
        Ok(CuckooFilter {
            table,
            scheme: Segmented3aryScheme { segment_size: segment_size },
            num_items: 0,
            max_kicks: MAX_KICKS_DEFAULT,
        })
    }
...
impl CuckooFilter<Segmented4aryScheme> {
    ...
    ///
    /// ```rust
    /// use segmented_cuckoo_filter::{CuckooFilter, Segmented4aryScheme};
    ///
    ...
    pub fn new(num_buckets: u32, bucket_size: u32, fingerprint_bits: u32) -> Result<Self, CuckooError> {
        ...
        Ok(CuckooFilter {
            table,
            scheme: Segmented4aryScheme { segment_size: segment_size },
            num_items: 0,
            max_kicks: MAX_KICKS_DEFAULT,
        })
    }
...
impl CuckooFilter<Standard2aryScheme> {
    ...
    ///
    /// ```rust
    /// use segmented_cuckoo_filter::{CuckooFilter, Standard2aryScheme};
    ///
    ...
    pub fn new(num_buckets: u32, bucket_size: u32, fingerprint_bits: u32) -> Result<Self, CuckooError> {
        ...
        Ok(CuckooFilter {
            table,
            scheme: Standard2aryScheme { num_buckets: num_buckets },
            num_items: 0,
            max_kicks: MAX_KICKS_DEFAULT,
        })
    }
...
impl CuckooFilter<Standard3aryScheme> {
    ...
    ///
    /// ```rust
    /// use segmented_cuckoo_filter::{CuckooFilter, Standard3aryScheme};
    ///
    ...
    pub fn new(num_buckets: u32, bucket_size: u32, fingerprint_bits: u32) -> Result<Self, CuckooError> {
        ...
        Ok(CuckooFilter {
            table,
            scheme: Standard3aryScheme { num_buckets: num_buckets },
            num_items: 0,
            max_kicks: MAX_KICKS_DEFAULT,
        })
    }
...
impl CuckooFilter<Standard4aryScheme> {
    ...
    ///
    /// ```rust
    /// use segmented_cuckoo_filter::{CuckooFilter, Standard4aryScheme};
    ///
    ...
    pub fn new(num_buckets: u32, bucket_size: u32, fingerprint_bits: u32) -> Result<Self, CuckooError> {
        ...
        Ok(CuckooFilter {
            table,
            scheme: Standard4aryScheme { num_buckets: num_buckets },
            num_items: 0,
            max_kicks: MAX_KICKS_DEFAULT,
        })
    }
...
impl<S: IndexScheme> CuckooFilter<S> {
    ...
    pub fn add<T: AsRef<[u8]>>(&mut self, item: T) -> Result<(), CuckooError> {
        ...
        for pos in 0..arity {
            if let Some(_slot) = self.table.insert_fingerprint_to_bucket(indices[pos], fingerprint)
            {
                self.num_items += 1;
                return Ok(());
            }
        ...
        // chain[i] = (bucket, slot, original_fingerprint)
        let mut chain: Vec<(u32, u32, u32)> = Vec::with_capacity(self.max_kicks as usize);

        for _ in 0..self.max_kicks {
            // Evict a random slot from cur_index
            let slot = rng.random_range(0..self.table.fingerprints_per_bucket);
            let evicted_fingerprint = self.table.read_fingerprint(cur_index, slot);
            let evicted_pos = self.scheme.position_of(cur_index);

            // Write cur_fingerprint into this slot
            chain.push((cur_index, slot, evicted_fingerprint));
            self.table.write_fingerprint(cur_index, slot, cur_fingerprint);
            ...
            for p in 0..arity {
                ...
                if let Some(_ins_slot) =
                    self.table.insert_fingerprint_to_bucket(all[p], evicted_fingerprint)
                {
                    self.num_items += 1;
                    placed = true;
                    break;
        ...
        for &(bucket, slot, original_fingerprint) in chain.iter().rev() {
            self.table.write_fingerprint(bucket, slot, original_fingerprint);
        }
    ...
    pub fn contain<T: AsRef<[u8]>>(&self, item: T) -> bool {
        ...
        let arity = self.scheme.arity();
        for i in 0..arity {
            if self.table.find_fingerprint_in_bucket(indices[i], fingerprint) {
                return true;
            }
        }
        false
    ...
    pub fn delete<T: AsRef<[u8]>>(&mut self, item: T) -> Result<(), CuckooError> {
        ...
        for i in 0..arity {
            if let Some(slot) = self.table.find_fingerprint_in_bucket_slot(indices[i], fingerprint)
            {
                self.table.write_fingerprint(indices[i], slot, 0);
                self.num_items -= 1;
                return Ok(());
            }
        }
    ...
    ///
    /// This counter is maintained exactly by `add` and `delete`. It is not affected by false
    /// positives.
    ///
    /// # Returns
    ///
    ...
    /// excluding the 8-byte alignment padding added by [`FingerprintTable`].
    ///
    /// # Returns
    ///
    ...
    /// # Performance
    ///
    /// O(1) — reads a field from [`FingerprintTable`].
    ///
    /// # Examples
    ///
    /// ```rust
    ...
    pub fn load_factor(&self) -> f64 {
        self.num_items as f64
            / (self.table.num_buckets as f64 * self.table.fingerprints_per_bucket as f64)
    }
    ...
    /// # Note — rollback instead of a victim cache
    ///
    /// When this budget is exhausted, [`add`](Self::add) rolls the in-progress chain of
