...
///
/// ```rust
/// use segmented_cuckoo_filter::hash::fingerprint_hash1;
///
/// let offset = fingerprint_hash1(0xABC, 256);
/// assert!(offset < 256);
/// ```
#[inline]
pub fn fingerprint_hash1(fingerprint: u32, range: u32) -> u32 {
...
///
/// ```rust
/// use segmented_cuckoo_filter::hash::fingerprint_hash2;
///
/// let offset = fingerprint_hash2(0xABC, 256);
/// assert!(offset < 256);
/// ```
#[inline]
pub fn fingerprint_hash2(fingerprint: u32, range: u32) -> u32 {
...
///
/// ```rust
/// use segmented_cuckoo_filter::hash::fingerprint_hash3;
///
/// let offset = fingerprint_hash3(0xABC, 256);
/// assert!(offset < 256);
/// ```
#[inline]
pub fn fingerprint_hash3(fingerprint: u32, range: u32) -> u32 {
...
///
/// ```rust
/// use segmented_cuckoo_filter::hash::extract_fingerprint;
///
...
/// ```
///
/// [`FingerprintTable`]: crate::bucket::FingerprintTable
#[inline]
...
#[inline]
pub fn fingerprint_hash_mod(fingerprint: u32, range: u32) -> u32 {
...
/// - `half` — segment size (`num_buckets / 2`); must be a power of 2.
...
///
/// ```rust
/// use segmented_cuckoo_filter::hash::hash_item_segmented_2ary;
///
/// let half = 64u32;
/// let (fingerprint, idx) = hash_item_segmented_2ary(b"hello", half, 12);
/// assert!(idx[0] < half);
/// assert!(idx[1] >= half && idx[1] < 2 * half);
/// ```
pub fn hash_item_segmented_2ary(item: &[u8], half: u32, fingerprint_bits: u32) -> (u32, [u32; 4]) {
    let (_h, fingerprint, i1) = hash_and_i1(item, fingerprint_bits, half);
    let i2 = half + (i1 ^ fingerprint_hash1(fingerprint, half));
...
///
/// ```rust
/// use segmented_cuckoo_filter::hash::{hash_item_segmented_2ary, all_indices_segmented_2ary};
///
/// let half = 64u32;
/// let (fingerprint, idx) = hash_item_segmented_2ary(b"hello", half, 12);
/// let from_second = all_indices_segmented_2ary(idx[1], fingerprint, half);
/// assert_eq!(from_second[0], idx[0]);
/// assert_eq!(from_second[1], idx[1]);
/// ```
pub fn all_indices_segmented_2ary(cur_index: u32, fingerprint: u32, half: u32) -> [u32; 4] {
    let h1 = fingerprint_hash1(fingerprint, half);
    if cur_index < half {
        let i1 = cur_index;
        let i2 = half + (i1 ^ h1);
        [i1, i2, 0, 0]
...
///
/// ```rust
/// use segmented_cuckoo_filter::hash::hash_item_standard_2ary;
///
...
///
/// ```rust
/// use segmented_cuckoo_filter::hash::{hash_item_standard_2ary, all_indices_standard_2ary};
///
...
/// }
/// ```
pub fn all_indices_standard_2ary(
    cur_index: u32,
    fingerprint: u32,
    num_buckets: u32,
) -> [u32; 4] {
...
mod tests {
    ...
    fn segmented_2ary_index_ranges() {
        let half = 128u32;
        for i in 0u32..1000 {
            let (_, idx) = hash_item_segmented_2ary(&i.to_le_bytes(), half, 12);
            assert!(idx[0] < half);
    ...
    fn segmented_2ary_all_indices_round_trip() {
        let half = 256u32;
        for i in 0u32..1000 {
            let (fingerprint, idx) = hash_item_segmented_2ary(&i.to_le_bytes(), half, 16);
            let a = all_indices_segmented_2ary(idx[0], fingerprint, half);
            assert_eq!(a[0], idx[0]);
            assert_eq!(a[1], idx[1]);
            let b = all_indices_segmented_2ary(idx[1], fingerprint, half);
            assert_eq!(b[0], idx[0]);
            assert_eq!(b[1], idx[1]);
        }
    }
    ...
    fn segmented_3ary_index_ranges() {
        ...
        for i in 0u32..1000 {
            ...
            assert!(idx[0] < segment_size, "i1={} must be < segment_size={}", idx[0], segment_size);
            assert!(idx[1] >= segment_size && idx[1] < 2 * segment_size);
            assert!(idx[2] >= 2 * segment_size && idx[2] < 3 * segment_size);
        }
    }
