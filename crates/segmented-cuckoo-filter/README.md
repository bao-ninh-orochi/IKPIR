...
- [Extension 2: Segmented Cuckoo Filters](#extension-2-segmented-cuckoo-filters)
...
`tag_hash_mod` required when n = 3^k (not a power of 2). The two entries also measure
...
partial-key hashing is not truly random: the xord offset `tag_hash(tag)` produces at most
