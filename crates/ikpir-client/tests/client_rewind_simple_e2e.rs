...
//! (`RewindClient`) against a real `IkpirServer` and `SimplePirBackend`.
//!
//! # Purpose
//!
//! Mirror of `client_rewind_e2e.rs`: pins the same client-side behaviours,
//! but against `SimplePirBackend` so the reshape + sparse-patch math gets
//! exercised end-to-end through the public client API.

use ikpir_client::{IkpirClientError, RewindClient, SimpleConfig, SimplePirBackend};
...
type Client = RewindClient<SimplePirBackend>;
