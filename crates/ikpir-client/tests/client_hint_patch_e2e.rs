//! End-to-end integration tests for the **client-hint-patch** flow
//! (`HintPatchClient`) against a real `IkpirServer` and `FrodoPirBackend`.
//!
//! # Purpose
//!
...
//! 7. `future_delta_resyncs_via_try_apply_delta_or_resync` — a delta that
//!    jumps more than one epoch ahead triggers a resync via the fetched
//!    fresh bundle, and the stale delta left behind is then rejected.
//!
//! Behaviours 1–6 are parameterised by arity (2 / 3 / 4) so the
//! diagnostic on failure pins which arity broke. Two extra
//! single-arity tests pin the constant-time-decode last-slot probe and
//! the `HintPatchMode` client preference. The wire-byte accounting
//! tests (`hint_delta_is_much_smaller_than_setup_bundle`,
//! `query_response_wire_sizes_match_underlying_vectors`) are
//! flow-independent and live only in `tests/client_rewind_e2e.rs`.

use ikpir_client::{
    DeltaApplyOutcome, FrodoConfig, FrodoPirBackend, HintPatchClient, HintPatchMode,
    IkpirClientError,
};
...
type Client = HintPatchClient<FrodoPirBackend>;
...
/// `HintPatchClient::from_setup(server.setup())` shortcut shared by every
/// arity-generic body.
...
/// A delta that jumps more than one epoch past the client resyncs via the
/// closure passed to `try_apply_delta_or_resync`: the outcome is
/// `Resynced`, the client's epoch catches up to the server's, a key
/// created by either of the two skipped mutations decodes correctly
/// post-resync, and the earlier (now doubly-stale) delta is rejected.
#[test]
fn future_delta_resyncs_via_try_apply_delta_or_resync() {
    let mut server = build_server_2();
    for k in 0u32..5 {
        let (key, val) = pair(k);
        server.insert(&key, &val).unwrap();
    ...
    let delta1 = server.insert(&k1, &v1).unwrap();
    let (k2, v2) = pair(501);
    let delta2 = server.insert(&k2, &v2).unwrap();

    // Only the second delta reaches the client: epoch jumps by 2, which
    // `apply_delta` alone would reject as `FutureDelta`.
    let outcome = client
        .try_apply_delta_or_resync(delta2, || server.setup())
        .expect("try_apply_delta_or_resync");
    assert_eq!(outcome, DeltaApplyOutcome::Resynced);
    assert_eq!(client.epoch(), server.epoch());

    // Both mutations landed before the resync's `server.setup()`, so a
    // lookup of either key decodes correctly post-resync.
    for (key, val) in [(k1, v1), (k2, v2)] {
        let q = client.build_query(&key);
        let resp = server.answer(&q).expect("answer ok");
        assert_eq!(
            client.decode(&key, &resp).expect("no error"),
            Some(val.to_vec()),
            "post-resync lookup of key {key:?} must reflect the skipped mutation"
        );
    }

    // The earlier delta is now doubly stale against the resynced client.
    let err = client.apply_delta(delta1).unwrap_err();
    assert!(
        matches!(err, IkpirClientError::StaleDelta { .. }),
        "got {err:?}"
