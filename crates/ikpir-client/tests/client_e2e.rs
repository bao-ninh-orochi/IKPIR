...
//! 3. `decode_after_accumulate_delta` — a client that accumulated deltas
//!    via `accumulate_delta` produces the same decodes as a
...
//! 5. `precomputed_survives_accumulate_delta` — warm queues stay
//!    coherent across `accumulate_delta`.
//! 6. `stale_delta_rejected` — `accumulate_delta` rejects a delta older
...
/// Arity-generic body of the headline correctness test: a client that
/// accumulated deltas via `accumulate_delta` decodes identically to a
/// freshly-rebuilt client, across a mixed insert/update/delete trace.
fn decode_after_accumulate_delta_inner<S>(mut server: IkpirServer<S, FrodoPirBackend>)
where
    S: IndexScheme + SchemeMeta + 'static,
{
...
/// precompute, accumulate a mixed mutation trace, then query — every
/// decode must match a freshly-rebuilt oracle client.
fn precomputed_survives_accumulate_delta_inner<S>(mut server: IkpirServer<S, FrodoPirBackend>)
where
    S: IndexScheme + SchemeMeta + 'static,
{
    for k in 0u32..15 {
...
/// A client that accumulated deltas via `accumulate_delta` matches a
/// freshly-rebuilt client across a mixed mutation trace (2-ary). This is
/// the headline correctness claim of the IKPIR scheme.
#[test]
...
/// `accumulate_delta` with an older-than-current epoch returns
/// `StaleDelta` (2-ary).
#[test]
