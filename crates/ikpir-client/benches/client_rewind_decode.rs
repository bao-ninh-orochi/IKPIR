//! Thin binary for the **client-rewind** decode bench. All measurement
//! logic lives in `flow_decode_body.rs` (`mod flow_decode_body;`, included
//! the same way `mod helpers;` is); this file only names the flow's client
//! type and wires up the arity/backend dispatch. See `flow_decode_body.rs`
//! for intent, method, CLI, and CSV documentation.
//!
//! **Output:** `results/ikpir_client_rewind_decode.csv`

mod flow_decode_body;
mod helpers;

use flow_decode_body::Cli;
use helpers::Backend;
...
fn dispatch_backend<S: helpers::MakeStore>(cli: &Cli, arity: u32, num_buckets: u32) {
    let lwe_dim = flow_decode_body::effective_lwe_dim(cli);
    match cli.backend {
        Backend::Frodo => flow_decode_body::run_one::<
            S,
            FrodoPirBackend,
            RewindClient<FrodoPirBackend>,
        >(cli, arity, num_buckets, FrodoConfig::with_lwe_dim(lwe_dim)),
        Backend::Simple => flow_decode_body::run_one::<
            S,
            SimplePirBackend,
